...
int checkScalarProcess(const char scalar[], int indexes[]) {
    ...
    static int index_of_first_symbol;
    if (scalar[index] == '\0') {
        return counter;
    }
    if (scalar[index] == '.') {
    ...
    if (index == 0 && !isalnum(scalar[index])) {
    ...
    } else if ((index == 0 || seen_dots == 2) && isdigit(scalar[index_of_first_symbol])) {
        found_constant = 1;
    } else if (seen_coma && scalar[index] == '.') {
        indexes[counter++] = index;
        ...
        indexes[counter++] = index;
    } else if (index == 1 && !isalnum(scalar[index - 1]) && scalar[index] == '.' && scalar[index + 1] == '.') {
        indexes[counter++] = index;
